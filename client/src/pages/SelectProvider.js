...
const SelectProvider = (props) => {
   ...
   useEffect(() => {
      console.log(parameters.get('specialty'));
   }, [parameters]);

   return (
      <main>
         <h1>Select Specialty and Provider</h1>
         <div>
            <Dropdown>
               ...
               <Dropdown.Menu className='specialtyList'>
                  {specialties.length > 0 &&
                     specialties.map((specialty) => (
                        <ul>
                           {' '}
                           <Link to={`?specialty=${specialty}`} key={specialty}>
                              {specialty}
                           </Link>
                        </ul>
                     ))}
               ...
               <tbody>
                  {providers.length > 0 &&
                     providers.map((provider) => (
                        <tr>
                           <td key={provider.username}>{provider.username}</td>
                           <td key={provider.specialty}>{provider.specialty}</td>
                           <td>
                              <Link to={`/selectdate/${provider.username}/${provider.specialty}`} key={provider._id}>
                                 Set an Appointment
                              </Link>
                           </td>
                        </tr>
